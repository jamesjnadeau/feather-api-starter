...
var outputContentToFile = function(hook, next) {
  ...
  //Call this when we are ready to writ the file
  var completeTask = function(err) {
    if(err) {
      next(err);
    }
  ...
  var parentDirName = pathUtil.dirname(outputFileName);
  mkdirp(parentDirName, '0744', function(err) {
      if (err) {
          if (err.code == 'EEXIST') completeTask(); // ignore the error if the folder already exists
          else completeTask(err); // something else went wrong
      } else completeTask(); // successfully created folder
  });
};
